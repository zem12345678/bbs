...
import (
	...
	"credit-service/internal/infrastructure/messaging"
	"credit-service/internal/infrastructure/persistence"
	creditgrpc "credit-service/internal/interfaces/grpc"
	"credit-service/internal/interfaces/grpc/pb/creditpb"
...
func main() {
	...
	creditpb.RegisterCreditServiceServer(server, creditgrpc.NewHandler(service))
...
func loadConfig(path string) (*config, error) {
	...
	if cfg.Service.Name == "" {
		cfg.Service.Name = "credit-service"
	}
	if cfg.Service.GRPCPort == 0 {
		cfg.Service.GRPCPort = 9107
	}
	if cfg.Postgres.DSN == "" {
		cfg.Postgres.DSN = "postgres://bbs_credit_app:local_credit_pass@127.0.0.1:5432/bbs?sslmode=disable&search_path=bbs_credit"
	}
	...
	if cfg.Kafka.UserGroupID == "" {
		cfg.Kafka.UserGroupID = "bbs-credit-user-consumer"
	}
	if cfg.Kafka.ArticleGroupID == "" {
		cfg.Kafka.ArticleGroupID = "bbs-credit-article-consumer"
	}
	if cfg.Kafka.CommentGroupID == "" {
		cfg.Kafka.CommentGroupID = "bbs-credit-comment-consumer"
	}
	if cfg.Kafka.ReactionGroupID == "" {
		cfg.Kafka.ReactionGroupID = "bbs-credit-reaction-consumer"
	}
	return &cfg, nil
}
