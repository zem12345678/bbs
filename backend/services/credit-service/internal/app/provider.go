...
import (
	...
	creditservice "credit-service/internal/application/credit"
...
func ProvideConsumerRunner(v *viper.Viper, kafkaOptions *iockafka.ConsumerOptions, projector *messaging.Projector, log logger.Logger) (*ConsumerRunner, error) {
	userReader, err := iockafka.NewConsumer(kafkaOptions.WithTopic(
		...
		StringDefault(v.GetString("kafka.userGroupId"), "bbs-credit-user-consumer"),
	))
	if err != nil {
		return nil, err
	}
	articleReader, err := iockafka.NewConsumer(kafkaOptions.WithTopic(
		...
		StringDefault(v.GetString("kafka.articleGroupId"), "bbs-credit-article-consumer"),
	))
	if err != nil {
		_ = userReader.Close()
		return nil, err
	...
	commentReader, err := iockafka.NewConsumer(kafkaOptions.WithTopic(
		...
		StringDefault(v.GetString("kafka.commentGroupId"), "bbs-credit-comment-consumer"),
	))
	if err != nil {
		_ = userReader.Close()
	...
	reactionReader, err := iockafka.NewConsumer(kafkaOptions.WithTopic(
		...
		StringDefault(v.GetString("kafka.reactionGroupId"), "bbs-credit-reaction-consumer"),
	))
	if err != nil {
		_ = userReader.Close()
		_ = articleReader.Close()
...
func (r *ConsumerRunner) Stop() error {
	...
	if firstErr != nil {
		return fmt.Errorf("close credit consumer: %w", firstErr)
	}
	return nil
}
