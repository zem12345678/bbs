...
func ResolveWorkerID(workerID, rangeStart, rangeSize int64, instanceName string) (int64, error) {
	...
	if instanceName == "" {
		if workerID < 0 || workerID > workerMax {
			return 0, fmt.Errorf("worker ID must be between 0 and %d", workerMax)
		}
		return workerID, nil
	}
	if rangeStart < 0 || rangeStart > workerMax {
		return 0, fmt.Errorf("worker ID range start must be between 0 and %d", workerMax)
	}
	if rangeSize <= 0 || rangeSize > workerMax+1-rangeStart {
		return 0, fmt.Errorf("worker ID range size must fit within 0 and %d", workerMax)
	}
