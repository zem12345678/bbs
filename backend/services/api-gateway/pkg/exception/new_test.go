...
func TestBusinessCodeHTTPStatus(t *testing.T) {
	tests := []struct {
		name       string
		err        *exception.ApiException
		wantStatus int
		...
		{
			name:       "访问令牌过期",
			err:        exception.NewAccessTokenExpired("token expired"),
			wantStatus: http.StatusUnauthorized,
		},
		{
			name:       "访问令牌不合法",
			err:        exception.NewAccessTokenIllegal("token invalid"),
			wantStatus: http.StatusUnauthorized,
		},
		{
			name:       "需要二次确认",
			err:        exception.NewVerifyCodeRequiredError("verify required"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.err.GetHttpCode(); got != tt.wantStatus {
				t.Fatalf("GetHttpCode() = %d, want %d", got, tt.wantStatus)
			}
		})
	}
}
