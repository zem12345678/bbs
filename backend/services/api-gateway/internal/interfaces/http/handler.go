...
type Handler struct {
	...
	pinActionLimit                     ratelimit.Limiter
...
func NewInitControllers(h *Handler) iochttp.InitControllers {
	return func(r *gin.Engine) {
		...
		h.registerNotesReadCompatRoutes(r)
		...
		for _, prefix := range []string{"/api", ""} {
			...
			r.POST(prefix+"/i/pin", h.requireAuthScope("write"), h.requireInteractiveAuth(), h.pinNote)
			r.POST(prefix+"/i/unpin", h.requireAuthScope("write"), h.requireInteractiveAuth(), h.unpinNote)
		...
		api.GET("/users/me/pinned", h.requireAuthScope("read"), h.listCurrentUserPinned)
