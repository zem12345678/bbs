...
func TestResolverBuildCreatesIndependentConnectionState(t *testing.T) {
	...
	second, err := builder.Build(resolver.Target{URL: url.URL{Path: "/mall/1.0.0"}}, secondConn, resolver.BuildOptions{})
	if err != nil {
	...
	if firstResolver.keyPrefix != "/user/1.0.0/" || secondResolver.keyPrefix != "/mall/1.0.0/" {
...
func TestResolverUpdateRefreshesSameAddressOnReregistration(t *testing.T) {
	...
	r.cc = cc
	info := Server{Name: "bbs-feed-service", Addr: "192.168.31.164:9113", Weight: 1}
