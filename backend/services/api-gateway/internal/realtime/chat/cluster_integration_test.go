...
func TestTwoRealtimeInstancesShareRedis(t *testing.T) {
	...
	publishDurableTestEvent(t, cleanupClient, "chat:user:"+strconv.FormatInt(userID, 10), "membership-left-1", "chat.membership.left.v1", userID)
	publishDurableTestEvent(t, cleanupClient, "chat:room:8", "membership-left-1", "chat.membership.left.v1", userID)
	readEventType(t, connectionA, "room.member.left")
	readEventType(t, connectionB, "room.member.left")
