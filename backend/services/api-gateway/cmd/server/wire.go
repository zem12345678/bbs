...
func CreateApp(configFile string) (*iocapplication.Application, error) {
	...
	blockingImportLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, time.Minute, 1)
	mutingImportLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, time.Minute, 1)
	followingImportLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, time.Minute, 1)
	userListImportLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, time.Minute, 1)
	noteImportLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, time.Minute, 1)
	pinActionLimit := ratelimit.NewRedisSlidingWindowLimiter(redisClient, 5*time.Second, 10)
	...
	handler.SetMutingImportLimit(mutingImportLimit)
	handler.SetFollowingImportLimit(followingImportLimit)
	handler.SetUserListImportLimit(userListImportLimit)
	handler.SetNoteImportLimit(noteImportLimit)
	handler.SetPinActionLimit(pinActionLimit)
