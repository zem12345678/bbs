...
func TestPostgresCollectionRepositoryIntegration(t *testing.T) {
	...
	collectionPage, keysetTotal, err := repo.ListCollectionsAfterID(ctx, ownerID, 0, 1)
	require.NoError(t, err)
	require.Equal(t, int64(2), keysetTotal)
	require.Len(t, collectionPage, 1)
	require.Equal(t, reading.ID, collectionPage[0].ID)
	collectionPage, keysetTotal, err = repo.ListCollectionsAfterID(ctx, ownerID, collectionPage[0].ID, 1)
	require.NoError(t, err)
	require.Equal(t, int64(2), keysetTotal)
	require.Len(t, collectionPage, 1)
	require.Equal(t, archive.ID, collectionPage[0].ID)
	collectionPage, _, err = repo.ListCollectionsAfterID(ctx, ownerID, collectionPage[0].ID, 1)
	require.NoError(t, err)
	require.Empty(t, collectionPage)
	...
	secondEntity := domain.EntityRef{Type: domain.EntityTopic, ID: entity.ID + 1}
	changed, err = repo.AddCollectionItem(ctx, ownerID, reading.ID, secondEntity)
	require.NoError(t, err)
	require.True(t, changed)
	itemPage, keysetTotal, err := repo.ListCollectionItemsAfterID(ctx, ownerID, reading.ID, "", 0, 1)
	require.NoError(t, err)
	require.Equal(t, int64(2), keysetTotal)
	require.Len(t, itemPage, 1)
	require.Equal(t, entity, itemPage[0].Entity)
	itemPage, keysetTotal, err = repo.ListCollectionItemsAfterID(ctx, ownerID, reading.ID, "", itemPage[0].ID, 1)
	require.NoError(t, err)
	require.Equal(t, int64(2), keysetTotal)
	require.Len(t, itemPage, 1)
	require.Equal(t, secondEntity, itemPage[0].Entity)
	itemPage, _, err = repo.ListCollectionItemsAfterID(ctx, ownerID, reading.ID, "", itemPage[0].ID, 1)
	require.NoError(t, err)
	require.Empty(t, itemPage)
	changed, err = repo.RemoveCollectionItem(ctx, ownerID, reading.ID, secondEntity)
	require.NoError(t, err)
	require.True(t, changed)
