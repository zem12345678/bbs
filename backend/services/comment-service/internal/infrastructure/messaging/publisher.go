...
func (p *KafkaEventPublisher) PublishDomainEvents(ctx context.Context, events []domain.DomainEvent) error {
	...
	for _, event := range events {
		...
		if err != nil {
			if p.log != nil {
				p.log.Error("marshal comment event failed", logger.String("event", event.EventName()), logger.Error(err))
			}
			continue
		}
		messages = append(messages, kafka.Message{
			...
			Headers: []kafka.Header{
				...
				{Key: "producer", Value: []byte("comment-service")},
			},
		})
	}
	if len(messages) == 0 {
	...
	if err := p.writer.WriteMessages(ctx, messages...); err != nil {
		return fmt.Errorf("publish comment events to kafka: %w", err)
	}
	return nil
}
