...
func TestGetNoteChartRejectsInvalidParametersAndMissingRepository(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewService(&noteChartRepoStub{}).GetNoteChart(context.Background(), test.query)
			if !errors.Is(err, test.want) {
				t.Fatalf("GetNoteChart() error = %v, want %v", err, test.want)
			}
		})
	}
	_, err := NewService(&commentRepoWithoutNoteChart{}).GetNoteChart(context.Background(), domain.NoteChartQuery{Span: "day", Limit: 1})
...
type commentRepoWithoutNoteChart struct{ domain.Repository }
