...
func TestGetActiveUsersChartNormalizesAndForwardsQuery(t *testing.T) {
	...
	got, err := NewService(repo).GetActiveUsersChart(context.Background(), domain.NoteChartQuery{Span: " HOUR ", Offset: &zero})
	if err != nil {
...
func TestGetActiveUsersChartRejectsInvalidParametersAndMissingRepository(t *testing.T) {
	...
	for _, test := range tests {
		_, err := NewService(&activeUsersChartRepoStub{}).GetActiveUsersChart(context.Background(), test.query)
		if !errors.Is(err, test.want) {
			t.Fatalf("GetActiveUsersChart() error = %v, want %v", err, test.want)
		}
	}
	_, err := NewService(&commentRepoWithoutActiveUsersChart{}).GetActiveUsersChart(context.Background(), domain.NoteChartQuery{Span: "day", Limit: 1})
...
type commentRepoWithoutActiveUsersChart struct{ domain.Repository }
