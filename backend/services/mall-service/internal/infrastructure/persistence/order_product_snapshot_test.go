...
func TestRefreshOrderFromCurrentProductsUsesLockedProductSnapshot(t *testing.T) {
	...
	if len(db.queries) != 1 || !strings.Contains(db.queries[0], "FOR UPDATE") {
		t.Fatalf("product query = %+v, want row lock", db.queries)
	}
	if len(db.args) != 1 || len(db.args[0]) != 1 || db.args[0][0] != int64(202) {
		t.Fatalf("product query args = %+v, want product 202", db.args)
	}
}

func TestRefreshOrderFromCurrentProductsLocksProductsInIDOrder(t *testing.T) {
	db := &orderProductSnapshotQueryer{
		products: map[int64]domain.Product{
	...
	if len(db.args) != 2 || db.args[0][0] != int64(1) || db.args[1][0] != int64(2) {
		t.Fatalf("product lock order args = %+v, want 1 then 2", db.args)
	}
}
...
func (q *orderProductSnapshotQueryer) Query(context.Context, string, ...any) (pgx.Rows, error) {
	return nil, errors.New("unexpected query")
}

func (q *orderProductSnapshotQueryer) QueryRow(_ context.Context, query string, args ...any) pgx.Row {
	q.queries = append(q.queries, query)
	q.args = append(q.args, args)
	if len(args) != 1 {
		return orderProductSnapshotRow{err: errors.New("expected product id arg")}
	}
	productID, ok := args[0].(int64)
	if !ok {
		return orderProductSnapshotRow{err: errors.New("expected int64 product id")}
	}
	product, ok := q.products[productID]
	if !ok {
		return orderProductSnapshotRow{err: pgx.ErrNoRows}
	}
	return orderProductSnapshotRow{product: product}
}
