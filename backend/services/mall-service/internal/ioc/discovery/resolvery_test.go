...
func TestResolverBuildCreatesIndependentConnectionState(t *testing.T) {
	...
	first, err := builder.Build(resolver.Target{URL: url.URL{Path: "/user/1.0.0"}}, firstConn, resolver.BuildOptions{})
	if err != nil {
	...
	if firstResolver.keyPrefix != "/user/1.0.0/" || secondResolver.keyPrefix != "/credit/1.0.0/" {
...
func TestResolverWatchRecreatesUnavailableChannelAfterSync(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			built, err := builder.Build(resolver.Target{URL: url.URL{Path: "/credit/1.0.0"}}, &fakeClientConn{}, resolver.BuildOptions{})
			if err != nil {
...
func TestResolverUpdateRefreshesSameAddressOnReregistration(t *testing.T) {
	...
	r.cc = cc
	info := Server{Name: "bbs-credit-service", Addr: "192.168.31.164:9107", Weight: 1}
