...
function getStorageItem(key) {
  const rawList = storageLocal().getItem<optionsItem[]>(key) || [];
  const list = sanitizeOptions(rawList);
...
function setStorageItem(key, value) {
  storageLocal().setItem(key, sanitizeOptions(value));
}

function isPermissionOption(item: any) {
  const type = String(item?.type ?? item?.meta?.type ?? "").toUpperCase();
  const menuType = Number(item?.menuType ?? item?.meta?.menuType);
...
function sanitizeOptions<T extends any[]>(value: T) {
  return Array.isArray(value)
    ? (value.filter(item => !isPermissionOption(item)) as T)
    : ([] as unknown as T);
}
...
function flatTree(arr) {
  ...
  function deep(arr, parentIcon?) {
    arr.forEach(item => {
      if (isPermissionOption(item)) return;
