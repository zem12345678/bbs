...
function hasOneShowingChild(children: menuType[] = [], parent: menuType) {
  const showingChildren = children.filter((item: any) => {
    if (isPermissionNode(item) || item?.meta?.showLink === false) {
      return false;
    }
...
function isPermissionNode(item: any) {
...
function singleRouteTarget(child: menuType, parent: menuType) {
  const target = child?.path ? child : parent;
  return {
    ...target,
    path: resolvePath(child?.path || parent?.path || "")
  };
}
...
<template>
  <SidebarLinkItem
    ...
    "
    :to="singleRouteTarget(onlyOneChild, item)"
  >
    <el-menu-item
