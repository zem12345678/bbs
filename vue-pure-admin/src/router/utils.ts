...
function asyncRoutesCacheKey() {
  const userInfo = storageLocal().getItem<DataInfo<number>>(userKey);
  const username = userInfo?.username ?? "anonymous";
  const roles = (userInfo?.roles ?? []).slice().sort().join(",");
  return `async-routes:v9:${username}:${roles}`;
}
...
function clearLegacyAsyncRoutesCache(currentKey: string) {
  ...
  const legacyKeys = [
    ...
    "async-routes:v4",
    "async-routes:v5",
    `async-routes:v1:${username}:${roles}`,
    `async-routes:v2:${username}:${roles}`,
...
function isPermissionNode(route: any) {
  const type = String(route?.type ?? "").toUpperCase();
  const menuType = Number(route?.menuType ?? route?.meta?.menuType);
  ...
  return (
    ...
    type === "B" ||
    menuType === 3 ||
    Object.prototype.hasOwnProperty.call(query, "perm") ||
    /(^|[/?#&])perm=/i.test(path) ||
    /(^|[/?#&])perm=/i.test(name)
  );
}

function sanitizeAsyncRoutes(routeList: any[] = []) {
  return routeList
    .filter(route => !isPermissionNode(route))
    .map(route => {
      const nextRoute = { ...route };
      if (Array.isArray(nextRoute.children)) {
        const children = sanitizeAsyncRoutes(nextRoute.children);
...
function handleAsyncRoutes(routeList) {
  const sanitizedRouteList = sanitizeAsyncRoutes(routeList);
  if (sanitizedRouteList.length === 0) {
    usePermissionStoreHook().handleWholeMenus(sanitizedRouteList);
  } else {
    formatFlatteningRoutes(addAsyncRoutes(sanitizedRouteList)).map(
      (v: RouteRecordRaw) => {
        // 防止重复添加路由
        if (
...
function isUsableTopMenu(route: menuType) {
  const path = String(route?.path ?? "");
...
function findFirstBackstageMenu(routes: menuType[] = []) {
  for (const route of routes) {
    const children = route?.children ?? [];
    const child = findFirstBackstageMenu(children);
    if (child) return child;
    if ((route as any)?.meta?.backstage && isUsableTopMenu(route)) {
      return route;
    }
  }
  return null;
...
function findFirstUsableMenu(routes: menuType[] = []) {
  for (const route of routes) {
    const children = route?.children ?? [];
    const child = findFirstUsableMenu(children);
    if (child) return child;
    if (isUsableTopMenu(route)) return route;
  }
  return null;
}
...
function getTopMenu(tag = false): menuType {
  ...
  const topMenu =
    findFirstBackstageMenu(wholeMenus) ??
    findFirstUsableMenu(wholeMenus) ??
    handleTopMenu(wholeMenus[0]?.children?.[0] ?? wholeMenus[0]);
