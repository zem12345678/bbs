...
function isPermissionTag(tag: any) {
  const type = String(tag?.type ?? tag?.meta?.type ?? "").toUpperCase();
  const menuType = Number(tag?.menuType ?? tag?.meta?.menuType);
...
function sanitizeTags(tags: any[]) {
  return Array.isArray(tags) ? tags.filter(tag => !isPermissionTag(tag)) : [];
}
...
export const useMultiTagsStore = defineStore("pure-multiTags", {
  ...
  actions: {
    ...
    handleTags<T>(
      ...
      switch (mode) {
        case "equal":
          this.multiTags = sanitizeTags(value as any[]);
          this.tagsCache(this.multiTags);
          break;
        case "push":
          {
            const tagVal = value as multiType;
            if (isPermissionTag(tagVal)) return;
            // 不添加到标签页
